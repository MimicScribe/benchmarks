...
**Aggregate SAA is unchanged at 98.1%.** The work in this cycle went into the ASR layer, and it shows up in what the pipeline no longer drops rather than in the headline number.

Missed speech fell from 8.2% to 6.6% of reference time, and it fell on every corpus: AMI 16.3% to 13.8%, DiPCo 25.9% to 24.1%, VoxConverse 5.5% to 4.0%, SCOTUS 2.4% to 0.7%, Earnings-21 0.7% to 0.1%. Confusion held flat at 1.9%. That pairing is the part worth reporting: capturing more mumbled and overlapped speech normally *raises* confusion, because speech that is never transcribed can never be misattributed.

The gain concentrates in short turns, which the previous publication named as the frontier this benchmark should be judged on improving. Attribution on reference turns under two seconds and clear of other speech rose from 52.9% to 60.4%. Turns of two to ten seconds rose from 84.1% to 86.8%. Turns spoken over someone else rose from 9.5% to 11.8%, still the hardest row in the table.

Substantial turns are unchanged. 44 of the 2,987 reference turns of ten seconds or longer land under the wrong speaker, against 42 in the previous run, and 43 of the 57 files still have none.
...
|--------|------:|----------------:|----------------:|--------:|
| Earnings-21 | 11 | 98.1% | **97.9%** | 3.7x |
| VoxConverse | 20 | 95.9% | **96.9%** | 4.1x |
| SCOTUS | 5 | 99.2% | **99.2%** | 5.4x |
| AMI | 16 | 97.0% | **97.9%** | 3.7x |
| DiPCo | 5 | — | **97.4%** | — |
| **Aggregate** | **57** | — | **98.1%** | — |

On the 52-file basis used before DiPCo was added, the aggregate is **98.2%**. Speedup is diarization-only on Apple M1 Max (ANE vs MPS GPU). **Pyannote C1** is the reference [community-1](https://huggingface.co/pyannote/speaker-diarization-community-1) pipeline run in Python with default parameters; the reference has not yet been run on DiPCo.
...
**Substantial turns.** Of the 2,987 turns of ten seconds or longer across the corpus, 44 (**1.5%**) end up under the wrong speaker's label. 43 of the 57 files have none. At five seconds or longer it is 3.2%. The misattributed turns concentrate in the hardest rooms: the two crowded, overlap-heavy meeting series (AMI's EN2002 four-way discussions and DiPCo's dinner parties) account for more than half of the 44.
...
| Over 10s | 2,983 | 98.5% |
| 2–10s | 6,601 | 86.8% |
| Under 2s, clear of other speech | 3,387 | 60.4% |
| Under 2s, spoken over someone | 4,646 | 11.8% |
...
**Confusion is the meaningful component, and it held flat at 1.9% aggregate** (1.8% on the 52-file basis). Per corpus: Earnings-21 2.0% to 2.1%, AMI 2.2% to 2.1%, VoxConverse 2.8% to 3.1%, SCOTUS 1.0% to 0.8%, DiPCo unchanged at 2.6%. Aggregate DER improves from 17.4% to 16.0%, and almost all of that is missed speech falling from 8.2% to 6.6%. That is ASR coverage, not attribution.
...
|--------|----:|----------:|------------:|-------:|
| Earnings-21 | 15.9% | 2.1% | 13.7% | 0.1% |
| VoxConverse | 14.8% | 3.1% | 7.7% | 4.0% |
| SCOTUS | 1.5% | 0.8% | 0.0% | 0.7% |
| AMI | 26.0% | 2.1% | 10.1% | 13.8% |
| DiPCo | 29.4% | 2.6% | 2.7% | 24.1% |
| **Aggregate** | **16.0%** | **1.9%** | **7.5%** | **6.6%** |
...
The deterministic pipeline runs end-to-end in Swift; Python only does scoring (and, separately, the LLM step — which does not produce the published score). This run was produced at `parakeet-transcriber` commit `3037eff0` with default parameters.
...
Aggregates are comparable only within the same file basis: 58 → 52 removed the Podcast corpus (May 2026); 52 → 57 added DiPCo (July 2026). On the shared 52-file basis, both the 2026-07-17 and 2026-08-25 runs score 98.2%.
